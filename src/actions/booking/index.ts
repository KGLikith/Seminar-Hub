...
export async function getMyBookings(clerkId: string) {
  return await prisma.booking.findMany({
    where: { teacher_id: clerkId },
    include: {
      hall: { include: { department: true } },
      hod: true,
...
export async function rejectBooking(bookingId: string, hodId: string, reason: string) {
  const booking = await prisma.booking.update({
    where: { id: bookingId },
    data: {
      status: "rejected",
...
export async function addBookingSummary(bookingId: string, summary: string, aiSummary?: any) {
